...
<div role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-sm">
            ...
            <tbody>
            <c:forEach items="${data.orders}" var="item">
                <tr id="tr_${item.id}">
                    <td>${item.id}</td>
                    <td>${item.name}</td>
                    <td>${item.price}</td>
                    <td>${item.groupNum}</td>
                    <td><fmt:formatDate value="${item.stopTime}" pattern="yyyy-MM-dd"/></td>
                    <td>
                        <fmt:formatDate value="${item.getTimeStart}" pattern="yyyy-MM-dd"/>
                        至
                        <fmt:formatDate value="${item.getTimeStop}" pattern="yyyy-MM-dd"/>
                    </td>
                    <td>${item.stateStr}</td>
                    <td>
